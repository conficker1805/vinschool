...
question_template = QuestionTemplate.create!(
  ...
  slim_content: <<~TEXT
    ...
    .clock
      ...
      .digital-watch
        .dw-hour.text-success.bold data-action="click->selector#openModal" data-modal-name="hourModal"  data-result=hour
        span = ':'
        .dw-minute.text-success.bold data-action="click->selector#openModal" data-modal-name="minuteModal" data-result=minute
    = render partial: 'shared/modals/selector', locals: { id: 'hourModal', options: (1..12).to_a }
    = render partial: 'shared/modals/selector', locals: { id: 'minuteModal', options: (0..55).step(5).to_a }
    scss:
