  ...
  Question.create!(
    ...
    options: {
      ...
      answers: [
        ...
        { text: "#{name} có thể mua #{texts.sample} có giá #{us_money_to_words([money_value - 0.5, 0.01].max)}", result: 'Đúng' },
        { text: "#{name} không thể mua #{texts.sample} có giá #{us_money_to_words([money_value - 0.5, 0.01].max)}", result: 'Sai' },
